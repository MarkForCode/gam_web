...
const getCaptcha = async () => {
  try {
    const data = await fetch(API_URL + `/api/v1/backstage/login/captcha/code`, );
    if (data.status == 200) {
...
const CaptchaInput: React.FC<CaptchaInputProps> = ({value = {}, onChange}) => {
  ...
  useEffect(() => {
    getCaptcha().then((data: any) => {
      const url = API_URL + '/api/v1/backstage/login/captcha?id=' + data.id;
      setCodeId(codeId);
      setUri(url);
  ...
  const onClickImage = async () => {
    ...
    setUri(API_URL + '/api/v1/backstage/login/captcha?id=' + response.id);
